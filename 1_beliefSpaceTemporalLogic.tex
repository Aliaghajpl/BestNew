...
%\author{Cristian-Ioan Vasile, Rohan Thakker, ..., Ali Agha, Richard Murray, Aaron Ames {\color{red}(ACM format)}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
    ...
    missions specificied in terms of temporal specifications requires deterministic models or perfect observability. %, assumes the low level temporal properties are deterministic (or perfectly-observable, if stochastic).
...
% 	$var(b(m^{i}|z_{0:k}))>threshold$
    ...
    \pr{Science} The science objective in this campaign is to collect two samples from high-valued science areas A and one sample from area B. Let $\pred_{X} = p^r \in X$ denote the event that pick up a sample from region $X$ that is subset of the terrain.
    \begin{align}
    ...
    \pr{Safety specification} Space application falls into the category of safety-critical applications. Thus, maintaining the safety of systems along the operation is a very important objective of the mission. We express the safety specification as:
	\begin{align}
    ...
    \pr{Dynamics predicates} Due to rover and copter mechanical/electrical capabilities, these assets should not exceed certain velocity levels given the environment status. The following specification captures such mission requirements.
    \begin{align}
...
A Feedback-based Information RoadMap (FIRM) is a graph in belief space $\TS = (\TSX, \TSE, B^0)$, where $\TS$ represents a set of nodes in belief space $\TSX$ and the edge corresponds to a feedback local controller $\mu^{ij} \in \TSE$, which drives the system from belief node $B^i$ to $\epsilon$-neighbourhood of $B^j$ in finite time. The initial belief node is denoted by $B^0$. Note that each FIRM edge is a local controller, i.e., $\mu^{ij}:\mathbb{B}\rightarrow\mathbb{U}$.
\end{definition}

FIRM iteratively grows a graph $\TS$ in belief space, where each vertex/node corresponds to a Gaussian belief state $b = (\hat{x},P) \in \TSX$ with mean $\hat{x}$ and covariance $P$ \cite{Agha14}. For more details about sampling-based algorithms, and their connection to fomral methods see~\cite{Lav06,KF-IJRR11,VaBe-IROS-2013}. %In Alg.~\ref{alg:compute-mdp}, edge controllers are generated using the method $localController()$.
...
The existence of satisfying policies results immediately from the construction. We only need to check if the product MDP $\PA$ has any accepting transitions $Acc_\PA$.
...
%}
{\color{red} define the value function...}
