...
\pr{Differential Flatness} The quadcopter dynamics are differentially flat. Thus, control input $u$ and state $x$ can be written as algebraic functions $\alpha$ of flat variables $x^f = [x, y, z, \psi] \in \mathbb{R}^3 \times SO(2)$ and their derivatives.

$(u_t, x_t) = \alpha(x_t^f,\dot{x}_t^f,\ddot{x}_t^f,\dddot{x}_t^f,\ddddot{x}_t^f)$

\pr{Policy}
Since the quadrotor system is deferentially flat, given a sufficiently smooth trajectory in flat space $x^f(t): t \to \mathbb{R}^3\times SO(2)$ we can obtain the trajectory in state space control inputs $u(t)$ and 
we have segments...
