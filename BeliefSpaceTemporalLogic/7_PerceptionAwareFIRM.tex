...
\begin{gather}
    ...
    \nonumber M = -k_R e_R - k_{\Omega} e_{\Omega} + \Omega \times J\Omega\\ - J (\hat{\Omega} R^T R_d \Omega_d - R^T R_d \dot{\Omega_d})
\end{gather}

\pr{Differential Flatness} The quadcopter model is differentially flat. Thus, we can find a control input $u(t)$ and state $\boldsymbol{x}(t)$ for a given trajectory of the copter in the flat space $x^f(t)$. $x^f = [x, y, z, \psi] \in \mathbb{R}^4$. 
