...
\label{eq:rovervalue}
\begin{aligned}
	& V_r(x_{t}^r, x_{t}^e, x_{t}^\varphi)  = \max_{\mu_1} \mathbb{P}_{\mu_1}^{\MDP_1} \left[\mathbf{x}^1_{[t,T_c+T_r]} \models \lozenge X_f \mid \mathbf{x}_{[t]} = x_{t} \right].
\end{aligned}
\end{equation}
...
\label{eq:value_iter_mission}
  \begin{align} 
    ...
    & \pol_1^t(x) \in \argmax_{u\in \U_{rov}} \left(\max( \ind_{X_f}(x), \mathbb{E}\left[V^{t+1}(x_{t+1}){\mid }x_t=x\right] ) \right)%\mathbb E \left[ \max( \ind_{X_f}(x'), V^{t+1}(x')) \right] .  
    \end{align}
\end{subequations}
