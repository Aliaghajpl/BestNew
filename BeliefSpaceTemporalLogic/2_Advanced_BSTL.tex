...
\todo[inline]{Sofie: Would it make sense to introduce a example here, explaining the Markov process notation. Perhaps specifically how we use it to catch the uncertainty in the environment?}
...
In the rest of the paper we will assume that all MDPs are finite, i.e. they have finite state and input spaces. \red{To handle continuous-state MDPs, there are principled methods to construct finite-state approximations in a principled way[double]. Furthermore, notions of simulation relations can be employed to quantify the difference between an abstraction and the concrete continuous-state system in a way that allows controllers designed for the finite-state abstraction to be implemented on the concrete system with preserved guarantees \cite{Zamani2015,Haesaert2017}.} \sofie{[replace with:] To handle continuous-state MDPs, there are principled methods to construct and use their finite-state approximations  \cite{Zamani2015,Haesaert2017}. }
 An alternative is to use approximate dynamic programming (also known as reinforcement learning) directly on the continuous state space \cite{Powell2011}. Approximate methods allow a trade-off between optimality and scalability.\sofie{This is for standard dynamic programming, most of the bounds require some type of contractivity (enforced by the discounting.)}
...
\end{aligned}
\end{equation}
This function can be computed via value iteration as explained in later sections. The copter cannot affect $x_r^{T_c}$ or $x_\varphi^{T_c}$ during its execution on $[0, T_c]$, but it can affect $x_e^{T_c}$ via exploration. We next discuss the copter's objective: given $V_r(x^{T_c}_r, x^{T_c}_e, x^{T_c}_\varphi)$, how should it explore the environment in order to maximize knowledge?
...
\subsection{Task specific information gathering}%{Knowledge Optimization}
