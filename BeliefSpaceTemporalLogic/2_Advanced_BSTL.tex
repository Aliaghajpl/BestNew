...
\todo[inline]{Remove initial distribution}
...
\label{def:MDP}
  ...
  \end{itemize}
\end{definition}
For a given sequence of control inputs $u_k \in \U$, we say that an \emph{execution} of $\MDP$ is a sequence of states $\mathbf x = x_0x_1x_2x_3\ldots $ for which the initial state is a realization of $\rho$, that is $x_0 \sim \rho$, and for which each state transition is obtained as a realization of the stochastic transition kernel $x_{k+1} \sim \tr(\cdot \mid x_k, u_k)$. 
A \emph{policy} $\pol = \pol_0 \pol_1 \ldots$ is a sequence of mappings $\pol_k:\prod_{k}\X\rightarrow \U$ from the state space to the action space.  A policy is \emph{Markov}  if for all $k$ it holds that  $\pol_k:\X\rightarrow \U$. A controlled execution is generated by $\pol$ if each state transition is obtained as a realization of the stochastic transition kernel $x_{k+1} \sim \tr(\cdot \mid x_k, \mu_k(x_0,\ldots,x_k))$. All systems in this paper are modeled as MDPs, or as combinations (products) of MDPs.
...
\end{aligned}
\end{equation}
Starting from $ V^{T_c+T_r}\equiv 0$, this probability can be inductively computed, via \emph{value iterations}, along with the optimal policy as
\begin{subequations}
...
\begin{aligned}
	...
	W_1(x_1, x_2) & = \mathbb{E}^{\MDP_1} \left[ W_2(x_1', x_2) \mid x_1, u_1   \right], \\
	\bellman V(x_1, x_2) & = g(x_1, x_2, W_1(x_1, x_2)).
\end{aligned}
\end{equation*}
% \begin{equation*}
% \begin{aligned}
...
\begin{aligned}
	& W_i (x_1', \ldots, x_{i-1}', x_i, \ldots, x_n) \\
	& = \mathbb{E}^{\MDP_i}\hspace{-0.5mm}  \left[ W_{i+1} (x_1', \ldots, x_{i-1}', x_i', \ldots, x_n) \hspace{-0.5mm} \mid\hspace{-0.5mm}  x_i, C_i(x_{i-1}') \right]
\end{aligned}
\end{equation}
% \begin{equation}
% \label{eq:general_decomp}
