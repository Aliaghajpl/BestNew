...
\sofieNew{For a given sequence of control inputs $u_k \in \U$, we 
 say that an \emph{execution} of $\MDP$ is a sequence of states $\mathbf x = x_0x_1x_2x_3\ldots $ %(x_0, u_0)(x_1, u_1)\ldots$ 
...
% don't talk about non Markov policies (its too difficult for the audience)
A \emph{Markov policy} $\pol = \pol_0 \pol_1 \ldots$ is a sequence of mappings from the state space to the action space $\pol_k:\X\rightarrow \U$.  %We say that a policy is \emph{Markov} if $\pol_k( \cdot \mid x_0, \ldots, x_{k}) = \pol_k( \cdot \mid x_{k})$ for all $k$. 
...
\end{figure} 
In fact, for any specification $\varphi$ in the scLTL fragment it is possible to define such a  finite-state automaton $\FSA_\varphi$ \cite{Belta2017} with accepting set $Q_f$.
...
%It is well known that the optimal reachability probability can be computed via value iteration of a Bellman operator \cite{Baier2008}. 
...
Equipped with the back-stepping method for value iteration on product systems, we turn back to the exploration problem, that is the specification-guided information gathering. 

Remark first that during the exploration phase we can model the overall stochastic as 
\begin{equation}
\begin{aligned} 
	\MDP_2 & = \MDP_{cop} \otimes_{ser}^{C_3} \left( \MDP_{e_1} \otimes_{par} \ldots \otimes_{par} \MDP_{e_n} \right),
\end{aligned}
\end{equation}
with  $C_3 : \X_{cop} \rightarrow \U_{e_1} \times \ldots \times \U_{e_n}$ a map from the  poses of the copter to measurements of the environment labels,
Hence during the exploration phase,  $\MDP_2$ will change the state of the belief environment $x_e$ at time $T_c$.
 
{\color{orange}We next discuss the copter's objective: given $V_r(x^{T_c}_r, \cdot, x^{T_c}_\varphi)$, how should it explore the environment in order to maximize knowledge (i.e. maximize the probability of a favorable $x_{env}^{T_c}$)?}
...
\red{[I have copied a lot of the following section into the above section. This section can now be pruned down and connected more to 
 solving the computational challenges (see problem statement)] }
 \todo[inline]{PRUNE}
