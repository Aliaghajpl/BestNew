...
Our approach lies at the intersection of formal synthesis methods and optimal control: we phrase the exploration problem as a two-stage optimal control problem in the combined space of robot poses and environment beliefs. To partially circumvent the curse of dimensionality in the resulting large system we leverage its decomposed nature and perform value iteration via sequential back stepping, which avoids construction of the overall system.
...
A Markov decision processes is defined as follows \cite{hll1996}.
...
A common way to represent a map for planning purposes is to attach labels such as ``sand'' or ``rock'' to different locations, but in a Mars setting the true labels are often unknown at the time of system deployment. We capture this environmental uncertainty with MDPs $\MDP_{e_i}$ that model belief dynamics over labels. Transitions in the MDPs occur when measurements are taken from on-board sensors, i.e. the robots need to be physically close to a region to obtain information about its true labels. 
...
To make things more concrete, consider a system consisting of the following components:
\begin{enumerate}
	\item An MDP $\MDP_{rov}$ modeling a ground rover in 2D space.
	\item An MDP $\MDP_{copt}$ modeling a copter in 3D space.
	\item An MDP $\MDP_{env}$ that captures belief dynamics over a collection of unknown environment labels.
...
We can now state the type of question we attempt to answer in this paper.
\begin{problem}
\label{prob:main}
...
Knowledge maximization is an interesting concept on its own, in Section \ref{sub:information} we will discuss the precise notion of knowledge we use in this paper. An advantage of considering the scLTL fragment is that policy synthesis for a scLTL specification over a space $\X$ is equivalent to reachability of $\X\times Q_f$ on the product system $\MDP \otimes_{ser} \FSA_\psi$, where $\FSA_\varphi$ is a finite-state automaton constructed from $\varphi$ \cite{Belta2017} with accepting set $Q_f$. A finite-state automaton is a special case of an MDP, thus also the product system is an MDP. It is well known that the optimal reachability probability can be computed via value iteration of a Bellman operator \cite{Baier2008}. In the following we discuss how we use reachability-based techniques to decompose the problem in a way that paves the way for optimized knowledge extraction.

In the rest of the paper we will assume that all MDPs are finite, i.e. they have finite state and input spaces. To handle continuous-state MDPs, there are principled methods to construct finite-state approximations in a principled way. Furthermore, notions of simulation relations can be employed to quantify the difference between an abstraction and the concrete continuous-state system in a way that allows controllers designed for the finite-state abstraction to be implemented on the concrete system with preserved guarantees \cite{Zamani2015,Haesaert2017}. An alternative is to use approximate dynamic programming (also known as reinforcement learning) directly on the continuous state space \cite{Powell2011}. Approximate methods allow a trade-off between optimality and scalability.


\section{Specification-Guided Exploration via Two-Stage Dynamic Programming}
\label{sec:solution}

Due to the nature of the problem (the copter is quick and has short battery life, the rover is slow), it is reasonable to consider a turn-based solution as we outline in Section \ref{sub:decompose}. In the statement of Problem \ref{prob:main} we were vague about what ``maximizing knowledge'' means, we discuss this further in Section \ref{sub:information} and phrase knowledge maximization as a reachability problem. Our proposed solution method requires value iteration over large MDPs. In Sections \ref{sub:products} and \ref{sub:valueit} we show how to exploit the factored system structure to partially circumvent scalability issues without resorting to approximations.
...
We decouple the problem by letting the copter execute first, followed by the rover. After converting the scLTL specification into a DFSA $\MDP_{\varphi}$ we consider the two separate systems illustrated in Figure \ref{fig:agg}: the goal of the copter system is to explore the uncertain environment in a way that maximizes knowledge about the satisfiability of the rover task. Consider the execution order depicted in Fig. \ref{fig:execorder} where the copter executes for a time $T_c$ followed by a rover execution for time $T_r$. These times can be informed e.g. by available battery capacity and remaining hours of daylight. We define a function $V_r$ that quantifies the probability that the rover will satisfy its specification before time $T_r + T_c$ as a function of the system state $(x^{T_c}_r, x^{T_c}_e, x^{T_c}_\varphi)$ at time $T_c$:
\begin{equation}
\label{eq:rovervalue}
\begin{aligned}
...
\end{aligned}
\end{equation}
This function can be computed via value iteration as explained in later sections. The copter can not affect $x_r^{T_c}$ or $x_\varphi^{T_c}$ during its execution on $[0, T_c]$, but it can affect $x_e^{T_c}$ via exploration. We next discuss the copter's objective: given $V_r(x^{T_c}_r, x^{T_c}_e, x^{T_c}_\varphi)$, how should it explore the environment in order to maximize knowledge?

\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			\draw[|-|] (0,0) node[yshift=-1mm, anchor=north] {$0$} -- (3,0) node[yshift=-1mm, anchor=north] {$T_c$};
			\draw[|-|] (3,0) -- (6,0) node[yshift=-1mm, anchor=north] {$T_c + T_r$};
			...
			\node at (3,0.6) {$x_e$};
			\node at (3,0.9) {$x_r$};
			\node at (3,1.2) {$x_\varphi$};

			\node at (6,0.6) {$x_e$};
			\node at (6,0.9) {$x_r$};
			\node at (6,1.2) {$x_\varphi$};
		...
		\end{tikzpicture}
	\end{center}
	\caption{Illustration of execution order. On $[0, T_c]$ the copter and environment systems execute, while on $[T_c, T_c + T_r]$ the rover, environment, and specification automaton execute.}
	\label{fig:execorder}
\end{figure}
...
We posit that the rover has a time window of length $T_r$ to fulfill its mission, and that the copter has battery sufficient to operate for a time $T_c$. In addition, the copter must land in a pre-designated landing area $X_l$ at the end of the execution.
