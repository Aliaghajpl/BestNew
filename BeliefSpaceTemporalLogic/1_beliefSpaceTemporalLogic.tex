...
Mars robot deployment that enforce constraints raging from science campaign
(e.g., getting science results from certain regions) to operational constraints
such as maintaining certain accuracy over the the vehicles' state and
...
together with an on-line execution procedure, where the exploration demands for
the helicopter are generated.
Exploration is necessary if insufficient knowledge is available to synthesize a
satisfying control policy with sufficient success probability.
Another case is during execution if a mismatch between maintained environment
map and the rover's sensor observations is detected, e.g., observing high-risk
areas with sand or high slopes.
