...
under the synthesized control policies.
To aid this objective, we devise a specification framework for missions based on
Distribution Temporal Logic (DTL), a language that is able to capture temporal
and uncertainty constraints.
Using DTL, we formally encode complex mission specifications used for actual
Mars robot deployment that enforce constraints ranging from science campaign
(e.g., getting science results from certain regions) to operational constraints
such as maintaining certain accuracy over the vehicles' state and
the environment, inter-agent distance, energy level, etc.

In this paper, we consider a simplified version of the full specification set.
...
In addition, a monitor is attached to the execution procedure that triggers re-planning if a mismatch between the maintained environment
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
    ...
    in its most generic and principled form as a Partially-Observable Markov Decision Process (POMDP) problem \cite{Leslie98} \cite{Smallwood73} \cite{Bertsekas2012}. For the high-level decision making problem we rely on Distribution Tempiral Logic for mission specification and techniques from formal methods for control policy synthesis. In this work, we aim to bridge the gap between these two layers and propose a unified mission and motion planning framework.
	
	In the rest of this section, we will go over over the related work and put our work into the context. 
	In Section \ref{sec:DTL}, we discuss the POMDP framework, belief space planning and propose a formal specification framework that can directly work with belief spaces for decision making. In Section \ref{sec:misspec}, we present DTL formulation of a full Mars mission.
	Next, we proceed to solve a simplified version of the problem that tackles the challenges of planning a rover with stochastic motion and perception
	and with partially known maps, reactive deployment, and coordination between the rover and helicopter for information gathering necessary for
	control policy synthesis.
% 	The rest of the section shows a general framework to formulate the mission specification into \DTL specification.
	Section \ref{sec:planner} details the process of synthesizing a control policy for unified mission and motion problem planning.
	...
	\item We propose a method that can handle failures and unexpected contingencies during the execution via re-planning and re-mapping the environment to get more information. The re-mapping phase is an instance of active perception behaviors that emerges of the proposed framework.
...
% ^.
    ...
    \label{pb:mpp}
    Let $\phi$ be a given DTL formula and let the system
...
% 	\end{figure}
	...
	\scalebox{.5}{\input{figs/speed_constraints.tex}}
...
To generate the FSA, we first convert \DTL to LTL simply by converting the predicates to symbols~\cite{JonesDTL2013},
...
A Feedback-based Information RoadMap (FIRM) is a graph in belief space $\TS = (\TSX, \TSE, B^0)$, where $\TS$ represents a set of nodes in belief space $\TSX$ and the edge corresponds to a local feedback controller $\mu^{ij} \in \TSE$, which drives the system from belief node $B^i$ to $\epsilon$-neighbourhood of $B^j$ in finite time. The initial belief node is denoted by $B^0$. Note that each FIRM edge is a local controller, i.e., $\mu^{ij}:\mathbb{B}\rightarrow\mathbb{U}$.
\end{definition}
...
and can be converted into a finite-memory policy on $\MDP$.
...
%\end{equation}
%
In other words, we find a policy that maximizes the estimated probability of visiting the $Acc_\PA$ states which can be done by finding the smallest fixed point of the Bellman equation.
...
\begin{figure*}[h]
	\begin{center}
		\includegraphics[trim=50 25 50 25,clip, width=0.225\textwidth]{figs/figure_1.png} ~ \includegraphics[trim=50 25 50 25,clip, width=0.225\textwidth]{figs/figure_3.png} ~
		\includegraphics[trim=50 25 50 25,clip, width=0.225\textwidth]{figs/figure_5.png} ~ \includegraphics[trim=50 25 50 25,clip, width=0.225\textwidth]{figs/figure_7.png} 
	\end{center}
	\label{fig:planning}
	\caption{Illustration of the incremental planning and re-planning procedure, where the shaded areas illustrate regions that have been explored by the copter. The first three figures illustrate how the roadmap is build until a trajectory that visits both \texttt{A} and \texttt{B} without entering \texttt{Obs} can be constructed. However, during execution (blue line), an additional obstacle \texttt{ObsLater} is encountered, which triggers a construction of a new roadmap. The rightmost figure shows the new roadmap together with an execution trajectory that fulfills the specification.}
\end{figure*}

\begin{figure*}[h]
	\begin{center}
		\includegraphics[width=0.15\textwidth]{figs/pdf1.png} ~ \includegraphics[width=0.15\textwidth]{figs/pdf2.png} ~
...
\begin{equation}
    \begin{bmatrix}
        ...
        y(t+1) \\
        \varphi(t+1)
    \end{bmatrix} = \begin{bmatrix}x(t) + v \cos(\varphi(t)) \\ y(t) + v \sin(\varphi(t)) \\ \varphi(t) + \omega \end{bmatrix} + w,
\end{equation}
where $v$ (velocity) and $\omega$ (angular velocity) are the inputs and $w \sim \mathcal N(0, Q)$ is Gaussian process noise. We assume availability of state observations corrupted by Gaussian noise, and use an extended Kalman filter to update the belief state of the system. To control the belief state between FIRM belief nodes, we use the same switching controller as in \cite{Vasile2016} that first directs the heading angle $\varphi$ towards the target before starting to move. 

In order to simulate an uncertain environment we assume that map labels are only available for the region surrounding the initial state. The rover can request that the copter explores a new area in order to extend the planning region. Furthermore, if the rover encounters a mismatch between the map and what it encounters during execution, the execution is aborted and a new plan is computed. These different phases of planning are illustrated in Figure \ref{fig:planning_overview}.

We defined a square environment with obstacles and science targets \texttt{A} and \texttt{B} in order to test the algorithm. Fig. \ref{fig:planning} illustrates the incremental construction of a road map, including a replanning step
...
\begin{tikzpicture}
	[mynode/.style = {draw, rounded corners, fill=blue!50, minimum width=12mm, minimum height=6mm}]
 	...
 	\draw[-latex] (solve) -- node[left] {$p < p^*$} (explore);
  	\draw[-latex] (solve) -- node[above] {$p\geq p^*$} (execute);
...
\caption{Illustration of the sample-solve-explore planning algorithm. A plan is not executed unless the probability of specification satisfaction is estimated to be above a threshold $p^*$. If the rover encounters unexpected terrain during the execution, the map is updated and a new plan is computed.}
\end{figure}
