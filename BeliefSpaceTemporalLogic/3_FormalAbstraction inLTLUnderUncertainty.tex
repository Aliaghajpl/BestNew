...
%    
    ...
    &\word_i \models  \phi_1 \Until \phi_2 & \Equiv\quad & \exists j \geq i \text{ s.t. } ( \BF{b}^j \models \phi_2 ) \\
    & & & \andltl (\BF{b}^k \models \phi_1, \forall k \in \{i, \ldots j-1\})\\
    &\word_i \models \Event \phi  & \Equiv\quad & \exists j \geq i \text{ s.t. } \BF{b}^j \models \phi \\
    &\word_i \models \Always \phi  & \Equiv\quad & \forall j \geq i \text{ s.t. } \BF{b}^j \models \phi
    \end{align*}
...
\begin{tikzpicture}
	\node (l0) at (-1,0) {};
	\node[circle, draw, fill=blue!40!gray]  (ll) at (0,0) {\textcolor{white}{$s_1$}};
		\node[circle, draw, fill=blue!40!gray] (rl) at (1.5,0) {\textcolor{white}{$s_2$}};
	\node[circle, draw, fill=blue!40!gray] (ru) at (2,1.5) {\textcolor{white}{$s_3$}};
	\node[circle, draw, fill=blue!40!gray]  (lu) at (0,2) {\textcolor{white}{$s_4$}};
\node[circle, draw, fill=gray]  (lut) at (3.7,.7) {\textcolor{white}{$s_5$}};
...
Consider a property in a subset of \DTL\!,  defined by
the following {\em syntax} 
    \begin{equation*}
     \phi :=  \True \ |\ f \leq 0% \ |\ \notltl \phi
      \ |\ \phi_1 \andltl \phi_2 \ |\ \phi_1 \Until \phi_2 \ |\ \Next \phi
