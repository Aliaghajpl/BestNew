...
{\color{blue} Can we put this in definition environment?}
...
%
Remark that \eqref{eq:beliefx} also can be rewritten as 
%\begin{align}
%	&&\hat x_{k|k}&=A\hat x_{k-1|k-1}+Bu_{k-1}+ \bar P  C^Ts_k+\Delta_k s_k
%\end{align}
%. 
\begin{align}
	&&\hat x_{k|k}&=A\hat x_{k-1|k-1}+Bu_{k-1}+  \bar P   C^T(  \hat{s}_k+s^\Delta_k)\notag\\&&&\qquad+\Delta_k( \hat{s}_k+ s^\Delta_k)\label{eq:beliefx2}
\end{align}
 with $\Delta_k:=(P_{k|k-1}C^T-  \bar P   C^T)$ and with $ \hat{s}_k\sim \CA N (0,\hat{S}_{inv})$ and $ s^\Delta_k\sim  \CA N (0,\  S_k^{-1}-\hat{S}_{inv}). $

We compute a simulation relation with respect to
  the lifted transitions 
	$\Wt((\hat x_k, \hat x_{k|k})\in \cdot| (\hat x_{k-1}, \hat x_{k-1|k-1}, \hat u_{k-1}))$ defined via \eqref{eq:abstract} and \eqref{eq:beliefx2},
and a simulation relation 
	\begin{align}\label{eq:rel}
\rel := \left\{(\hat x_k,b_k)| (\hat x_k-\hat x_{k|k})^T M(\hat x_k,-\hat x_{k|k})\leq \eps, \right.\\\qquad\left.  P^-\preceq P_{k|k} \preceq   P^+ \mbox{ with } b_k=(\hat x_{k|k}, P_{k|k} ) \right\}.\notag
	\end{align}

The abstract belief model $\hat\MB$ \eqref{eq:abstract} can be further abstracted as to a finite state model $\hat\MB_{grid}$ with states $s\in S=1,2,..., $. Each state $s$ is associated to a representative point $x_s\in \Xb$ and associated to
...
where $\hat t$ is the stochastic transition kernel associated with $\hat\MB$.

Suppose that the interface is defined as
\[\InF(\hat u, \hat x, \hat x_{\,|\,}):=K( \hat x_{\,|\,} -\hat x)+\hat u.\]
We can quantify the difference between $\MB$ and $\hat\MB_{grid}$via \eqref{eq:rel} by verifying that for all  $(\hat x_k,\hat x_{k|k})\in \rel$ with probability at least $1-\delta$ it holds that $(\hat x_{k+1},\hat x_{k+1|k+1}\in \rel$. The difference expression in \eqref{eq:}
evolves as
\begin{align}
	\hat x_{k+1}-\hat x_{k+1|k+1}:= 
\end{align}

\begin{figure}[htp]
\centering
	\includegraphics[width = .8\columnwidth]{figs/grid}
	\caption{Gridded state space}
\end{figure}
