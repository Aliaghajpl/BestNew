...
\mathbf{P_{\! rob}}_{\tilde \init}^{\!\ast}(\tilde \MDP\vDash\psi; \tilde\Labset,\delta)\leq  \mathbf P _{\init}^{\polb}(\MDP\vDash\psi).
\end{align}
 \end{prop}
...
for $(\tilde x, x ) \in \rel$ and with $\mathbf V^\polb_{N-1} (x)$ computed with equation \eqref{eq:Valfunc} for $\MDP$, and  with $\mathbf W^{\tilde \polb}_{N-1} (x)$ computed with the robust operator $\mathbf R^{(\tilde \Labset, \delta)}_{\tilde\pol^\ast}$.
 \end{cor}
...
Remark that as a corollary of Proposition \ref{prop:1}, it also follows that the probability that the scLTL property is satisfied is still lower bounded as in \eqref{eq:prob_result}.
