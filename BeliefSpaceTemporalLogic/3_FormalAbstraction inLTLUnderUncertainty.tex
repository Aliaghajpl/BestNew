...
\begin{definition}
		\new{Borel measurable stochastic kernels $\mathbf{t}_1(\cdot \mid x)$ and $\mathbf{t}_2(\cdot \mid x)$ are in a \ $\delta$-lifted relation $\bar \rel_\delta$ for a set $x\in A$, i.e.,  $\mathbf{t}_1(\cdot \mid x) \bar \rel_\delta \mathbf{t}_1(\cdot \mid x)\, \forall x\in A$,  if there exists a lifting $\mathbb W_t$ that is a  Borel measurable stochastic kernel for which Definition \ref{def:dellifting} holds. }
\end{definition}
