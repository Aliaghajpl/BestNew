...
\rel := \left\{(s,b)| (\hat x-x_s)^T M(\hat x-x_s)\leq \eps, \right.\\\qquad\left.  P^-\preceq P \preceq   P^+ \mbox{ with } b_k=(\hat x, P ) \right\},\notag
	\end{align}
...
for some matrices $M, K,P^+,P^-$.  \new{We require that $P^+$, and $P^-$ are an upper, respectively, lower bound for $P_{k|k}$ of the belief MDP \eqref{eq:beliefx}-\eqref{eq:beliefP}. }
