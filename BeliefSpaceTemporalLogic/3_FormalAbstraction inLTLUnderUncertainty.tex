...
we know  that there exists a refined control policy with inherited guarantees.\begin{prop}\label{prop:1}
  ...
  $\tilde{\MB}\preceq^{\delta}_{\tilde{\Labset},\Labset}  \MB$, then a control policy ${\tilde \polb}^\ast$ for $\tilde \MB$ can be refined to a control policy $\polb$ for $\MB$ such that %for all joint executions $\mathbf{\tilde b}$ and $\mathbf{b}$
  \begin{equation}
...
Additionally, we can assume without loss of generality that no measurability issues are introduced as the maximization is performed over a subset of the simpler state space of the abstracted model.} Remark that as a corollary of Proposition \ref{prop:1}, it follows that the probability that an scLTL property is satisfied is still lower bounded as in inequality \eqref{eq:prob_result}.
...
As a first abstraction, we choose to reduce the complexity and dimensionality of the dynamics by neglecting the variations of covariance matrix $P$. Therefore,  $\tilde \MB$ is obtained by replacing the stochastic transitions in equation \eqref{eq:beliefx} by
\begin{equation}
		\tilde x_k  = A\tilde x_{k-1} +B\tilde u_{k-1} + \tilde P  C^T  \tilde{s}_k,\label{eq:abstract}
\end{equation}
with $ \tilde{s}_k\sim \CA N (0,\tilde{S}_{inv})$ and $\tilde{S}_{inv}\preceq S_k^{-1}$ for all $k$, where $\tilde P$ defines the steady state value of  $P_{k|k-1}$, i.e., the solution of the Kalman equations. We say that system $\tilde \MB$ \eqref{eq:abstract} has state space $\Xb_{x}$. \new{It is now of interest to show that there exists a value for $\delta$ such that  $\tilde \MB\preceq^\delta_{\tilde \Labset,\Labset} \MB$.} \new{Remark that, if this holds it follows that for  the resulting LTI MDP  system, we can  grid  $\Xb_x$  as in \citep{haesaert2017verification,tech_report_TACAS}  to obtain a finite state  MDP $\tilde \MB_{grid}$ over which we can do the control synthesis. Via transitivity of Definition \ref{def:apbsim}, the latter finite state MDP system will then also be in an $\delta$- approximate stochastic simulation relation. The details of this step have been omitted.}
...
\new{To compute  $\tilde \MB\preceq^\delta_{\tilde \Labset,\Labset} \MB$, } we consider the simulation relations $\rel$ between states $b=(\hat x, P)$ of $\MB$ and states $\tilde x$ of $\tilde \MB $ and  interface $\InF$ defined as
\begin{align}
  \label{eq:rel}
...
For a position-based proposition $p_i$ consider, without loss of generality, a labeling $\Lab_{p_i}: \Xb\rightarrow \{\{p_i\},\emptyset\}$ for the concrete belief MDP defined by $p_i\in\Lab_p \left( (\hat x,P) \right)  \Leftrightarrow \hat x \in A$. The set-valued extension to $2^{\{\{p_i\},\emptyset\}}$ for the abstract MDP is defined as
 ...
 \end{align}
The labeling $\tilde \Labset_{p_i}$ can be easily computed by shrinking or expanding the set $A$ and the extension to multiple atomic propositions is straightforward. Similarly, for propositions involving the variance of the current belief state any atomic property that is \textcolor{orange}{monotonic\footnote{\color{orange}Here monotonicity of a function in $P$ is defined based on the preorder $\succeq $ for the matrices $P$ with $A\succeq B$ if $x^T(A-B)x\geq0\, \forall x\in\mathbb R^n$. } in $P$} can be mapped to the abstract model. However, atomic propositions that include probability require caution since the quantification of probability of an event is in general not \red{monotonic} with respect to variance.
...
We can quantify the $\delta$-difference between $\MB$ and $\tilde \MB$ \eqref{eq:abstract} by verifying that for all  $(\hat x_k,\hat x_{k|k})\in \rel$, with probability at least $1-\delta$ it holds that $(\hat x_{k+1},\hat x_{k+1|k+1})\in \rel$. The difference between the states in \eqref{eq:abstract} and \eqref{eq:beliefx} evolves according to
\begin{equation}
\begin{aligned}
...
%\end{aligned}
%\end{equation}
\new{We can bound the norm of the noise terms ${s}^\Delta_{k+1}$ and $ \tilde{{s}}_{k+1}$ using probabilistic guarantees computed via inter alia the chi-square distribution.} 
\new{The computation of the values of $\epsilon$ and the values of the matrices  in $\rel$ \eqref{eq:rel} and in the interface \eqref{eq:intf}, can now be performed via sequential convex optimizations expressed with linear matrix inequalities. }

 
\begin{remark}
  Here we did not explicitly write out the lifted probability space $\mathbb{W}_t$ of Definition \ref{def:kernellift}. It is obtained from the probability measure induced by joint evolution of \eqref{eq:beliefx} and \eqref{eq:abstract} driven by noise sources $s^\Delta$ and $ {\tilde{s}} $.  \end{remark}
%
%{\color{red}
%\begin{itemize}
...
For the satisfaction of \eqref{eq:sr-1}, we repeat the same procedure as above and show that there exists an initial state $\tilde x_{0}$ (or if needed a distribution) for $\tilde \MB$ such that  $\tilde x_{0} \bar\rel_\delta \initb$. %, which is true if  $P^-\preceq  P_\init \preceq P^+$ and $x_{s,0}\in \rel^{-1}( \hat x_\init )$. %For the latter not to be an empty set it is sufficient if for all $x$ with $x \in \ball_\grid(0)$ it holds that $x^T M x \leq \eps^2$.
...
A finite abstraction $\MDP_{syst}$ \red{(Or $\MB_{syst}$)} is constructed as outlined in the previous section by discretizing the state space $[-10, 10]^2$ with a discretization parameter $\grid = (0.76481, 0.64426)$, and by representing the input space $[-1,1]^2$ with nine discrete inputs $\{0, 1, -1\}^2$.

{\color{red}
Say something about Fig. \ref{fig:tradeoff}}
{\color{orange} Fig.  \ref{fig:tradeoff} give the trade of between the $\epsilon$ error in the relation \eqref{eq:rel} and the $\delta$-difference.  Combined with a relaxing of the labeling as introduced in the previous subsection, this represents the accuracy of the used gridded belief MDP, that is $\tilde \MB_{grid}\preceq^\delta_{\tilde\Labset_{grid},\Labset} \MB $. For the evaluation of this simulation relation, we have also incorporated the accuracy loss caused by the gridding procedure. .}
\begin{figure}
  \input{figs/epsdel} 
  \caption{Trade-off between $\epsilon$ and $\delta$ for simulation relation.   }
  \label{fig:tradeoff}
\end{figure}
