...
Consider the choice of $\tilde f =f $  for $f(\cdot):=\det(\cdot)+c_1$, this can be of interest if $\tilde P\succeq P$ implies that
...
Let $\mathcal F$ be a set of predicates
\[\mathcal F:= \{f_i \mbox{ for } i\in [0, K]\}\]
\begin{definition}[$\delta$-predicate simulation relation]\label{def:apbsim}
Consider a concrete MDP $\MDP$ and an abstract  MDP $\tilde\MDP$, with predicate sets $\mathcal F$ and $\tilde {\mathcal{F}}$.   
We say that	$\tilde\M$ is $\delta$-stochastically simulated by $\M$, that is $\tilde{\M}\preceq^{\delta}_{\tilde{\mathcal{F}},\mathcal{F}}\M$, with respect to $(\tilde{\mathcal{F}},\mathcal{F})$  if there exists an interface function $\InF$ and
	a Borel measurable relation $\rel\subseteq \X_1\times \X_2$, for which \textbf{SR.1-2} hold and for which for $i \in \{1,\ldots, K\} $  
	\begin{equation}
	  \forall (\tilde b,b)\in \rel: \tilde f(\tilde b)\leq 0\rightarrow f( b)\leq 0.
\tag{\textbf{SR.~$\CA F$}}
	\end{equation} 
with $\tilde f_i \in \tilde{\mathcal{F}},$ and $f_i\in \mathcal F$. \hfill\mbox{ }\qed
\end{definition}
