...
\label{sec:gaussian}
\red{[I am going to remove the details of the abstraction in this section, it is not a new contribution and it complicates the explanation. It makes more sense to explain that the labeling based relations are transitive and show that since we can go to a standard Gaussian process, we can afterwards use any method for abstracting it to a finite state model. ]}
...
\end{align}
where $s_k\sim \mathcal N (0, S_k ),$ and
\begin{equation}
\begin{aligned}
