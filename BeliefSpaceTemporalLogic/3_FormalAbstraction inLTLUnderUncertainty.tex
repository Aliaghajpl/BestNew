...
(\word \in\Language (\mathcal A_\psi))$  over the traces $\word$ of $\MB$ to a reachability problem
 over another MDP,  denoted  $\MB\otimes\mathcal A_\psi$, which we refer to as a product of the MDP $\MB$ and the automaton $\mathcal A_\psi$. This was originally derived in \citep{tmka2013} for MDPs. We give a similar definition of this product construction.
%
...
\{\bar x(t)\}_{t\geq 0}$  represents an accepted word for specification $\psi$ if the trace reaches the set of accept states $K:=\X\times Acc$.
This reachability property can be evaluated over a   trajectory with length $N_s$ as 
\begin{align*}\textstyle
\exists j \in [0,N_s]: \bar x(j) \in K.\end{align*}
The probability associated to this finite-time reach-avoid event can be characterized as a boolean expression using indicator functions \cite{Abate1},
which leads to 
%its expression as 
an expectation over the state trajectories as 
\newcommand{\ind}{\mathbf{1}}
\begin{align*} 
r_{\bar x }^\polb(K)=\mathbb{E}_{\bar x(0)}^{\bar \polb}\bigg[\sum\limits_{\mathclap{\qquad j\in[0,N_s]}}\ind_{K}(\bar x(j))\prod_{i=0}^{j-1}\ind_{\bar{\X}\setminus K}(\bar x(i))\bigg],
\end{align*} 
...
For a given
%property and
policy $\polb$, the time-dependent value function $\!\mathbf V^\polb_k:\X\rightarrow [0,1]$,  is defined as 
\begin{align*}\mathbf V_k^\polb(\bar x)=\mathbb{E}^\polb\!\left[\,\sum\limits_{\mathclap{\qquad \ j\in [k+1,N_s]}} \,\, \ind_K(\bar x(j)) \prod\limits_{\mathclap{i=k+1}}^{j-1}\ind_{\bar{\X}\setminus K}(\bar x(i))\bigg|\bar x(k)=\bar x\right]\!,\end{align*}
...
This function allows expressing the discounted reach-avoid probability backward recursively,
as follows. 
...
\begin{align}
\mathbf V^\polb_{k-1} = 
 \Bel_\pol \mathbf V^\polb_{k}  \end{align}
Thus if $\mathbf V^\polb_k$ expresses the probability of reaching the accept states in $N_s-k$ then $ \Bel_\pol \mathbf V^\polb_{k} $ expresses the probability of reaching them in $N_s-k+1$.
In other words, every recursion combines the probability to reach the accept states with the probability of reaching it in the future.
For the product MDP, this Bellman operator reduces to 
\begin{align}\label{eq:V_recopt_inf}
& \Bel_\pol (V)(x,q) =\!\!\int_{\bar\X}  \left[\mathbf 1_{Acc}(q')+  \mathbf 1_{Q\setminus Acc}( q')V( x', q'))\right]\notag\\&\hspace{3cm}\hfill 
...
\axx{For explanation for the T operator, you can divide it to two pieces an explain the meaning of each piece.} 
