...
\end{equation}
with $ \tilde{s}_k\sim \CA N (0,\tilde{S}_{inv})$ and $\tilde{S}_{inv}\preceq S_k^{-1}$ for all $k$, where $\tilde P$ defines the steady state value of  $P_{k|k-1}$, i.e., the solution of the Kalman equations. We say that system $\tilde \MB$ \eqref{eq:abstract} has state space $\Xb_{x}$ and abstract the system further to a finite-state model $\tilde \MB_{grid}$ by gridding $\Xb_x$. 
...
We consider simulation relations $\rel$ between states $(\hat x, P)$ of $\MB$ and states $\tilde x$ of $\tilde \MB $ and interface $\InF$ defined on the forms
\begin{align}
  \label{eq:rel}
