...
\cristi{C: $\tr\left(\cdot\mid x_k, u_k \right)$ ?}
...
%that is partially observable via
...
\end{align}
\cristi{C: The first $\cdot$ in the lhs of eq.~\eqref{eq:trb} is a subset of $\X$, while the second $\cdot$ in the rhs is a subset of $\Xb$. It is confusing as it written now.}
...
discrete-time MDP defined by the tuple $\MB(\POMDP) = (\Xb, \initb, \trb, \A)$ with
  \begin{itemize}
    ...
    \item $\initb$, the initial probability distribution $\initb:=\init$; \cristi{C: Shouldn't we have $\initb \in \Xb$?}
    \item $\A$, the set of control inputs with $u\in\A$ as its elements;
...
Thus a policy for $\MB(\POMDP)$ %\axx{has $\MB$ defined as a standalone symbol?}
 is a sequence $\polb=(\pol_0,\ldots,\pol_{N-1})$ such that  for all $k$, $\pol_k(du_k|\init, i_k)$ is a universally measurable stochastic kernel on $\A$  \new{parameterized with the initial state distribution and the information sequence, that is  $(\init, i_k)\in \mathcal{P}(\X)\times \I_k$.} \marginpar{\tiny \axx{I have a little bit of challenge with this product space here.} better?}
