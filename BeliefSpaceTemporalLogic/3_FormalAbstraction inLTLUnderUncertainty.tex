...
\begin{align}
  \label{eq:rel}
    & \rel = \left\{(\tilde x, (\hat x, P ) ) \Big{|}\begin{aligned}
      ...
      P^-\preceq P \preceq   P^+
    \end{aligned} \right\}, \\
    & \InF(\tilde u,  \tilde x, \hat x)  =K( \hat x - \tilde x)+\tilde u,
\end{align}
...
\begin{equation}
\begin{aligned}
 & \hat x_{k+1|k+1}-  \tilde x_{k+1}=(A+BK)(\hat x_{k|k}-\tilde x_{k}) \\
 &\qquad\qquad  +  \bar P   C^T s^\Delta_{k+1} +\Delta_{k+1}( \red{\hat{s}}_{k+1}+ \red{s}^\Delta_{k+1}),
\end{aligned}
\end{equation}
...
%\end{aligned}
%\end{equation}
Given that $(\hat x_{k|k}-\hat x_{s|k})$ %and $x_\grid$ 
belongs to bounded sets, we can bound the influence of the noise terms $\red{s}^\Delta_{k+1}$ and $ \hat{\red{s}}_{k+1}$   to arrive at a probability $1-\delta$ that lower bounds the probability of remaining in $\rel$.
\begin{remark}
