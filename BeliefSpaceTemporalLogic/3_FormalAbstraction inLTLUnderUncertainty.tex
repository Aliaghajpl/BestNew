...
For this, we use an approximate  abstraction %of the belief space model
 suitable for the correct-by-design control synthesis. By designing a control policy over the abstract model and refining it back to the original model, the computational issues involved with control synthesis on the original model can be circumvented. 
...
We solve the synthesis problems in a correct-by-design fashion. We leverage the result in \citep{haesaert2017verification, tech_report_TACAS}, as such we can precede  discretization %to a finite abstraction
with model order reductions and simplifications.  Since we work over belief MDPs, this is crucial.  Further, we also define an approximate stochastic simulation relation via non-determinism in the labelling, and we newly refine the control policy via the abstract value function.
...
Consider a Markov decision process \citep{Bertsekas2012,mt1993,hll1996}, defined as follows.%
...
% {\color{blue}A little confusing to use $\Labset$ for both language and labelling map}  => fixed
