...
\begin{definition}[Markov decision process (MDP)]\label{def:MDP} \mbox{ }\\
 ...
 \end{definition}
 \red{[measure/distribution $=>$ measure (troughout the paper ]}
...
and an initial state  $x_0$ (sampled from distribution $\init$ \axx{or $\rho$?}),
the state $x_{k+1}$
is obtained as a realization of the controlled Borel-measurable stochastic kernel $\tr\left(\cdot{\,\mid\,t} x_k, u_k \right)$ --
...
\axx{why $r$? in robotic-pomdp literature, this is not common.}
\end{enumerate}
...
discrete-time MDP defined by the tuple $\MB(\POMDP) = (\Xb, \initb, \trb, \U)$ with
  ...
  \end{itemize}
  for which $\Xb$ and $\U$ are  Polish spaces \citep{bertsekas2004stochastic} and $\trb$ is a Borel measurable mapping. \axx{can you assume Polish-ness for teh belief space?}
